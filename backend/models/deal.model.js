...
Deal.find = (data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        deal.price_new, deal.price_low, deal.price_ship, deal.store_id, deal.deal_url, deal.image_urls, deal.pinned as pinned,
        ...
        WHERE ${filter} ORDER BY pinned DESC, start_date DESC, deal.id ASC LIMIT ? OFFSET ?;`,
            [length, start_at],
            (err, rows) => {
                if (err) {
                    reject(err);
